...
test("Party context keeps actor, selected subject, and requested subject separate", () => {
  const input = { value: JSON.stringify({ loggedInPartyId: "100", selectedPartyId: "200" }) };
  const doc = { location: { href: "https://example.test/page?ID=300" }, querySelector: () => input };
  assert.deepEqual(getPartyContext(doc), { loggedInPartyId: "100", selectedPartyId: "200", requestedPartyId: "300" });
});
...
test("settings values must match their declared field types", () => {
  ...
  assert.doesNotThrow(() => assertSettingsMatchSchema({ second: 2, first: 1 }, { enum: [{ first: 1, second: 2 }] }));
});
